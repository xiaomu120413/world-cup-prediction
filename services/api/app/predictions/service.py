...
DEFAULT_SMALL_MODEL_VERSION = "small_outcome_2026_06_24_gbdt_context_v4"
DEFAULT_SCORELINE_MODEL_VERSION = "scoreline_poisson_context_2026_06_24_calibrated_v2"
DEFAULT_PREDICTION_MODEL_VERSION = DEFAULT_SMALL_MODEL_VERSION
DEFAULT_SCORELINE_TRAINING_SEED = 20260617
SCORELINE_VALIDATION_START = datetime(2023, 1, 1, tzinfo=ZoneInfo("UTC"))
...
class BaselinePredictionService:
    ...
    def recompute_small_outcome(
        ...
        scoreline_runtime = self.ensure_scoreline_runtime(
            model_version=DEFAULT_SCORELINE_MODEL_VERSION,
            seed=DEFAULT_SCORELINE_TRAINING_SEED,
        )
        snapshot_id = self.create_snapshot(
            runtime.model_version_id,
            scope,
            seed_value,
            notes=(
                f"Generated by {runtime.model_family}; "
                f"scoreline_model={scoreline_runtime.model_version}; "
                f"feature_snapshot_as_of={feature_result['snapshot_as_of_at']}"
            ),
        )
        match_rows = self.load_matches(match_ids, scope=scope)
        ...
        written_matches = 0
        for row in match_rows:
            prediction = self.build_small_outcome_prediction(row, runtime, scoreline_runtime)
            self.write_match_prediction(row.match_id, snapshot_id, runtime.model_version_id, prediction)
            written_matches += 1
        ...
        return {
            ...
            "model_version": runtime.model_version,
            "model_family": runtime.model_family,
            "scoreline_model_version": scoreline_runtime.model_version,
            "prediction_snapshot_id": str(snapshot_id),
            "feature_snapshot": feature_result,
            "matches_written": written_matches,
            "rankings_written": ranking_count,
    ...
    def ensure_scoreline_runtime(self, model_version: str, seed: int) -> ScorelineRuntime:
        ...
        search_train_goals, _heldout_goals = split_scoreline_examples_by_date(
            goal_examples,
            train_end=SCORELINE_VALIDATION_START,
            test_start=SCORELINE_VALIDATION_START,
        )
        final_train_goals, test_goals = split_scoreline_examples_by_date(
            goal_examples,
            train_end=TRAIN_END,
        ...
        validation_scorelines = [
            ...
            if SCORELINE_VALIDATION_START <= example.played_at < TRAIN_END
        ]
        test_scorelines = [example for example in scoreline_examples if example.played_at >= TEST_START]
        if not search_train_goals or not final_train_goals or not test_goals:
        ...
        feature_schema = {
            ...
            "training_config": {
                "validation_start": SCORELINE_VALIDATION_START.date().isoformat(),
    ...
    def ensure_small_outcome_runtime(self, model_version: str, seed: int) -> SmallOutcomeRuntime:
        ...
        history_core = train_lightgbm_outcome_model(
            base_train_examples,
            trees=180,
            learning_rate=0.035,
            max_depth=3,
        ...
        if raw_context_train_examples and raw_context_test_examples:
            ...
            active_model = train_lightgbm_outcome_model(
        ...
        scored = [
            {
                ...
                "champion_prob": simulation["champion_probabilities"].get(row.team_id, 0.0),
                "semifinal_prob": simulation["semifinal_probabilities"].get(row.team_id, 0.0),
                "round32_prob": simulation["round32_probabilities"].get(row.team_id, 0.0),
    ...
    def load_tournament_team_rows(self) -> list[Any]:
        return self.db.execute(
            select(
    ...
    def prediction_snapshot_seed(self, snapshot_id) -> int:
    ...
    def tournament_team_score(row, group_path: dict[str, Any] | None = None) -> dict:
        return trained_tournament_team_score(row, group_path, DEFAULT_TOURNAMENT_RANKING_PARAMS)
    ...
    def build_group_state_distributions(self, snapshot_id) -> dict[str, list[tuple[float, list[dict[str, Any]]]]]:
        rows = self.db.execute(
            select(
        ...
        stage_codes: dict[Any, str] = {}
        for row in rows:
            grouped.setdefault(row.stage_id, []).append(row)
            stage_codes[row.stage_id] = str(row.stage_code).split("-")[-1].upper()

        distributions: dict[str, list[tuple[float, list[dict[str, Any]]]]] = {}
        for stage_id, stage_rows in grouped.items():
            ...
            distributions[stage_codes[stage_id]] = enumerate_group_table_states(stage_rows, remaining_matches)
